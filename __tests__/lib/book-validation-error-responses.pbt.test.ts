 ...
 * For any invalid book creation or update request (missing required fields, invalid URLs,
 * invalid category, invalid data types), the API SHALL return HTTP status 400 with a
 * descriptive error message listing all validation failures.
 * 
...
describe('Property 2: Validation Error Responses', () => {
  describe('Create Book Schema - Error Message Quality', () => {
    it.prop([fc.constant({})])('returns descriptive errors for completely empty payload', (payload) => {
    ...
    it.prop([
      fc.string().filter(cat => {
        ...
        return trimmed.length > 0 && !BOOK_CATEGORIES.includes(trimmed as any)
      })
    ])('returns descriptive error for invalid category', (invalidCategory) => {
      const payload = {
        title: 'Valid Title',
        author: 'Valid Author',
    ...
    ])('returns descriptive error for invalid coverImage URL', (invalidUrl) => {
      const payload = {
        title: 'Valid Title',
        author: 'Valid Author',
    ...
    it.prop([
    ...
    ])('returns descriptive error for invalid purchaseUrl', (invalidUrl) => {
      const payload = {
        title: 'Valid Title',
        author: 'Valid Author',
        category: 'Spiritual Growth',
      ...
      if (result.success) {
        throw new Error('Validation should have failed for invalid purchaseUrl')
      }
      
      const urlError = result.error.issues.find(e => e.path[0] === 'purchaseUrl')
      if (!urlError) {
        throw new Error('Should have purchaseUrl error')
      }
      
      if (!urlError.message || !urlError.message.toLowerCase().includes('url')) {
        throw new Error('purchaseUrl error message should mention URL')
      }
    })

    it.prop([
      fc.string({ minLength: 201, maxLength: 300 }).filter(s => s.trim().length > 200)
    ])('returns descriptive error for title exceeding max length', (longTitle) => {
      const payload = {
        title: longTitle,
        author: 'Valid Author',
        category: 'Spiritual Growth',
  ...
  describe('Update Book Schema - Error Message Quality', () => {
    it.prop([fc.constant({})])('returns descriptive error for empty update payload', (payload) => {
    ...
    ])('returns descriptive error for invalid category in update', (invalidCategory) => {
      const payload = {
        category: invalidCategory
      }
    ...
    ])('returns descriptive error for invalid coverImage URL in update', (invalidUrl) => {
      const payload = {
        coverImage: invalidUrl
      }
  ...
  describe('Error Message Structure', () => {
    it.prop([
      fc.record({
        ...
        author: fc.constant(undefined),
        category: fc.constant(undefined),
        description: fc.constant(undefined),
        coverImage: fc.constant(undefined)
      })
    ])('provides error messages with field paths for missing fields', (payload) => {
      const result = createBookSchema.safeParse(payload)
    ...
    ])('provides unique error messages for different validation failures', (payload) => {
      const result = createBookSchema.safeParse(payload)
